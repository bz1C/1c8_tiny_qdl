...
// Модуль ББР_СтроковыеКлиентСервер. Версия 1.7 от 31.07.2023
...
Функция СтрЗаканчиваетсяНа_БезУчетаРегистра(знач Стр, знач Конец) Экспорт
	ДлинаКонца = СтрДлина(Конец);
	
	Возврат СтрСравнить(Прав(Стр, ДлинаКонца), Конец) = 0;
КонецФункции

Функция СтрЗаканчиваетсяСимволомИзСтроки(знач Стр, знач НаборСимволов) Экспорт
	Возврат СтрНайти(НаборСимволов, Прав(Стр,1)) <> 0;
КонецФункции
...
// Проверяет, что <ПреобразуемоеЗначение> - число в виде строки и если это так, записывает в <Результат> его значение.
// 
// Параметры:
//  Результат - Число - результат преобразования ПреобразуемоеЗначение в число
//  ПреобразуемоеЗначение - Строка - Преобразуемое значение
//  ВозможенРазделительТочка - Булево - учитывать и считать корректными ситуации, когда разделитель дробной части - '.'
// 
// Возвращаемое значение:
//  Булево - Истина, если в ПреобразуемоеЗначение является числом, указанным в виде строки
Функция ПроверитьПолучитьЧислоИзСтроки(Результат, знач ПреобразуемоеЗначение, знач ВозможенРазделительТочка = Истина) Экспорт
	Если ПустаяСтрока(ПреобразуемоеЗначение) или ТипЗнч(ПреобразуемоеЗначение) <> Тип("Строка") Тогда
		Возврат Ложь; 
	КонецЕсли; 
	...
	Если ВозможенРазделительТочка = Истина и СтрНайти(ПреобразуемоеЗначение,".") > 0 и СтрНайти(ПреобразуемоеЗначение,",") = 0 Тогда
